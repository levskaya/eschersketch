...
(function() {
  ...
  AffineTransform = (function() {
    ...
    AffineTransform.prototype.onvec = function(x) {
      var nx, ny;

      nx = this.a * x[0] + this.b * x[1] + this.x;
      ny = this.c * x[0] + this.d * x[1] + this.y;
      return [nx, ny];
    };
  ...
  transformAffineSet = function(transformAf, Afset) {
    var Af, invtransformAf, newAfset, _i, _len;

    newAfset = [];
    invtransformAf = transformAf.inverse();
    for (_i = 0, _len = Afset.length; _i < _len; _i++) {
      Af = Afset[_i];
  ...
  generateLattice = function(spec, nx, ny, d, phi, x, y) {
    var globalRot, i, j, transset, vec0, vec1, _i, _j, _ref, _ref1, _ref2, _ref3;
  ...
  generateTiling = function(spec, nx, ny, d, phi, x, y) {
    var Afset, glideset, globalRot, i, j, refs, refset, rots, rotset, transset, vec0, vec1, wholeset, _i, _j, _ref, _ref1, _ref2, _ref3;
  ...
  planarSymmetries = {
    ...
    p6: {
      rots: [[2 * PI / 3, sqrt(3) / 2, -0.5], [4 * PI / 3, sqrt(3) / 2, -0.5], [PI / 3.0, 0.0, 0.0], [-PI / 3.0, 0.0, 0.0], [3 * PI / 3.0, 0.0, 0.0]],
      refs: [],
      vec0: [sqrt(3) / 2, -1.5],
      vec1: [sqrt(3), 0.0]
    },
